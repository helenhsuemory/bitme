...
export default function Settings() {
  ...
  const handleResetProfile = () => {
    if (window.confirm('Nuclear Reset: This will reset EVERYTHING (links, services, profile, availability) to the initial template. All current changes will be lost. Proceed?')) {
      const { authUser, authLoading, toasts, ...data } = seedData;
      dispatch({ type: 'SET_STATE', payload: data as any });
      
      setDisplayName(seedData.user.displayName);
      setTitle(seedData.user.title);
      setBio(seedData.user.bio);
      setSocialLinks({ ...seedData.user.socialLinks });
      setSelectedThemeId(seedData.user.themePrefs.themeId);
      setPrimaryColor(seedData.user.themePrefs.primaryColor);
      
      addToast('Account reset to defaults');
    }
  };
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="flex items-center bg-slate-50 dark:bg-background-dark border border-slate-200 dark:border-primary/20 rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-primary/50 focus-within:border-primary transition-all">
                <span className="px-4 py-2.5 text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-primary/10 border-r border-slate-200 dark:border-primary/20 text-sm">helenhsu.vercel.app/</span>
                <input
                  type="text"
                  value={username}
